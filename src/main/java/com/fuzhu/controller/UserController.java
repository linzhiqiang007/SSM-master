...
public class UserController {
    ...
    public String insertUser(String account, String country) {
        User user = new User();
        long random_id = (long) (Math.random()*100);
        user.setId(random_id);
        user.setAccount(account);
        user.setCountry(country);
        //user.setAccount("shabaitian");
        //user.setCountry("中国");
        long random_money = (long) (Math.random()*100);
        user.setPayMoney(random_money);
...
//                    JSONArray array = JSONArray.parseArray(iter.next());
    ...
    @RequestMapping(value = "/FindAll", method = {RequestMethod.GET, RequestMethod.POST}, produces = "text/html;charset=UTF-8")
    public String FindAll(String account) {
        List<Score> userScores = userService.FindAll();
        for (Score score : userScores) {
            System.out.println(score.getScore());
            System.out.println(score.getUser());
        }
        return JSON.toJSONString(userScores);
    }

    @RequestMapping(value = "/FindAllByAccount", method = {RequestMethod.GET, RequestMethod.POST}, produces = "text/html;charset=UTF-8")
    public String FindAllByAccount(String account) {
        List<Score> userScores = userService.FindAllByAccount(account);
        List<UserScore> userScore1 = new ArrayList<UserScore>();
        for (Score score : userScores) {
            UserScore userScore = new UserScore();
            userScore.setUser(score.getUser());
            userScore.setScore(score);
            userScore1.add(userScore);
            //System.out.println("possword: " + score.getUser().getPassword() + " " + new Date());
        }
        return JSON.toJSONString(userScore1);
    }
}
